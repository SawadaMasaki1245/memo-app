...
editButton.addEventListener("click", clickAddMemo);
saveButton.addEventListener("click", clickAddMemo);
...
function setActiveStyle(index: number, isActive: boolean) {
  ...
  const element = document.querySelector(selector) as HTMLDListElement;
  if (isActive) {
...
//保存ボタンが押された時の処理
function clickSaveMomo(event: MouseEvent) {}
//メモデータを更新する
